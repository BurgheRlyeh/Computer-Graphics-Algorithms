	...
	for (int i{}; i < 4; i++) {
		{
			DirectX::XMFLOAT3 point{
				cameraPos.x - m_boundingRects[0].v[i].x,
					cameraPos.y - m_boundingRects[0].v[i].y,
					cameraPos.z - m_boundingRects[0].v[i].z,
			};
			float length = point.x * point.x + point.y * point.y + point.z * point.z;

			d0 = DirectX::XMMax(d0, length);
		}


		{
			DirectX::XMFLOAT3 point{
				cameraPos.x - m_boundingRects[1].v[i].x,
					cameraPos.y - m_boundingRects[1].v[i].y,
					cameraPos.z - m_boundingRects[1].v[i].z,
			};
			float length = point.x * point.x + point.y * point.y + point.z * point.z;

			d1 = DirectX::XMMax(d1, length);
		}
	}
