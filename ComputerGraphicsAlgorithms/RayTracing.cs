...
cbuffer AABBs: register(b4)
{
    AABB bbs[2 * LIMIT_I - 1 - 165];
}

cbuffer LFCPs: register(b5)
{
    int4 lfcps[2 * LIMIT_I - 1];
}

cbuffer TrianglesIDs: register(b6)
{
    int4 triIdx[LIMIT_I];
}
...
Intsec bestBVHLeafIntersection(Ray ray, int nodeId)
{
    ...
    for (int i = 0; i < lfcps[nodeId].z; ++i)
    {
        int mId = triIdx[lfcps[nodeId].y + i].x / 12;
        int tId = triIdx[lfcps[nodeId].y + i].x % 12;
...
Intsec bvhIntersection(Ray ray)
{
    ...
    while (stackSize > 0)
    {
        ...
        if (rayIntersectsAABB(ray, bbs[nodeId]) == whnf.w)
            continue;

        if (lfcps[nodeId].z == 0)
        {
            stack[stackSize++] = lfcps[nodeId].x;
            stack[stackSize++] = lfcps[nodeId].x + 1;
            continue;
        }
...
{
    return lfcps[nodeId].w;
}

int sibling(int nodeId)
{
    int parent = lfcps[nodeId].w;
    int left = lfcps[nodeId].x;
    int right = lfcps[nodeId].x + 1;
...
{
    int left = lfcps[nodeId].x;
    float tLeft = rayIntersectsAABB(ray, bbs[nodeId]);
    
    int right = lfcps[nodeId].x + 1;
    float tRight = rayIntersectsAABB(ray, bbs[nodeId]);
...
Intsec bvhStacklessIntersection(Ray ray)
{
    ...
    if (rayIntersectsAABB(ray, bbs[0]) == whnf.w)
        return best;
    ...
    for (int2 nodeState = int2(nearChild(0, ray), 0); nodeState.x != 0;)
    {
        ...
        if (nodeState.y == 0)
        {
            float t = rayIntersectsAABB(ray, bbs[nodeState.x]);
